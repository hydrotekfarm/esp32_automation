...
void measure_ec(void *parameter) {				// EC Sensor Measurement Task
	...
	for (;;) {
		if(sensor_calib_status(&ec_sensor)) { // Calibration Mode is activated
			ESP_LOGE(MQTT_TAG, "EC Wet Calibration Started");
			calibrate_sensor(&ec_sensor, &calibrate_ec, &dev);
			sensor_set_calib_status(&ec_sensor, false);
			if (!is_grow_active) {
				ESP_LOGE(MQTT_TAG, "EC task Suspended");
				vTaskSuspend(*sensor_get_task_handle(&ec_sensor));
			}
			ESP_LOGE(MQTT_TAG, "EC Wet Calibration Completed");
		} if(dry_calib) {
			ESP_LOGE(MQTT_TAG, "EC Dry Calibration Started");
			...
			ESP_LOGE(MQTT_TAG, "EC Dry Calibration Completed");
