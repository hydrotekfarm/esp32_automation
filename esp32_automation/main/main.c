...
#include "../components/ec_sensor/ec_sensor.h"
...
void measure_ec(void * parameter){
	...
	for(int i = 0; i < 20; i++){
		...
		printf("%f\n", voltage);
		ec = readEC(voltage, temp);
		printf("%f\n", ec);
		printf("\n\n");
		vTaskDelay(pdMS_TO_TICKS(2000));
	}

	esp_err_t er = calibrateEC();
	if(er != ESP_OK){
		ESP_LOGE(TAG, "%d", error);
	}
...
void app_main(void)
{
	...
	if((eventBits & WIFI_CONNECTED_BIT) != 0){
		...
		xTaskCreatePinnedToCore(measure_ec, "ec_task", 2500, NULL, 2, &ec_task_handle, 1);
	}
